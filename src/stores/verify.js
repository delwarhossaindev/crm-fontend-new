...
export const useVerifyStore = defineStore("verifyStore", {
  ...
  actions: {
    async fetchData(url, method = 'get', router = null) {
      const config = {
        headers: {
      ...
      try {
        const response = await axios[method](`${apiBase}${url}`, config);
    ...
    getSaleVerified(id, router) {
      return this.fetchData(`/sale/verified/${id}`, 'get', router);
    },
    getSaleReturnVerified(id, router) {
      return this.fetchData(`/sale-return/verified/${id}`, 'get', router);
    },
    ...
    getPurchaseVerified(id, router) {
      return this.fetchData(`/purchase/verified/${id}`, 'get', router);
    },
    ...
    getPurchaseReturnVerified(id, router) {
      return this.fetchData(`/purchase-return/verified/${id}`, 'get', router);
    },
  },
