  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center">
        <div class="mb-3">
          <h6 class="font-medium">Create New Win Probabilities</h6>
        </div>
        <div class="mb-3">
          <button
            type="button"
      ...
      <form @submit.prevent="submitForm">
        <div class="lg:grid grid-cols-3 gap-4 items-center">
          <!-- WinProbabilities Name -->
          <label for="name"
            >Win Probabilities Name <span class="text-red-600">*</span></label
          >
...
function validateForm() {
  const errors = {};
  if (!form.value.name) errors.name = "Win Probabilities Name is required";
...
const submitForm = async () => {
  if (validateForm()) {
    loading.value = true;
    try {
      const response = await winProbabilities.insertWinProbabilities(form.value);

      if (response?.status === 201) {
        showNotification(
          "success",
          response?.data?.message || "Win Probabilities successfully created."
        );
      ...
      if (error.response) {
        showNotification(
          "error",
          error.response.data.message || "Failed to create win probabilities."
        );
