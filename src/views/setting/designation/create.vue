  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center">
        <div class="mb-3">
          <h6 class="font-medium">Create New Designation</h6>
        </div>
        <div class="mb-3">
          <button
            type="button"
      ...
      <form @submit.prevent="submitForm()">
        <div class="lg:grid grid-cols-3 gap-4 items-center">
          <!-- Designation Name -->
          <label for="name"
            >Designation Name <span class="text-red-600">*</span></label
          >
          <input
            id="name"
            type="text"
...
const submitForm = async () => {
  if (validateForm()) {
    loading.value = true;
    try {
      const response = await designation.insertDesignation(form.value);
