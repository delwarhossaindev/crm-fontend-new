  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Job Type</h6>
...
const fetchJobTypeDetails = async () => {
  try {
    ...
    const response = await jobType.showJobType(id);
    form.value = response.data;
  } catch (error) {
    console.error("Failed to fetch job type details:", error);
  }
};
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await jobType.updateJobType(form.value, id);

    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      router.push({ name: "job-type" });
    }
  }
  catch (error) {
    if (error.response) {
      showNotification("error", error.response.data.message || "Failed to update job type");
