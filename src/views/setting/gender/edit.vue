  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Gender</h6>
...
const validateForm = () => {
  const errors = {};
  if (!form.value.name) errors.name = "Gender Name is required";
  formErrors.value = errors;
  return !Object.keys(errors).length;
};

const fetchGenderDetails = async () => {
  try {
    const { id } = route.params;
    const response = await gender.showGender(id);
    form.value = response.data;
  } catch (error) {
    console.error("Failed to fetch gender details:", error);
  }
};
...
const submitForm = async () => {
  ...
  try {
    ...
    if (error.response) {
      ...
      showNotification("error", error.response.data.message || "Failed to update gender");
