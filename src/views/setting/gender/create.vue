  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center">
        <div class="mb-3">
          <h6 class="font-medium">Create New Gender</h6>
        </div>
        <div class="mb-3">
          <button
            type="button"
      ...
      <form @submit.prevent="submitForm">
        <div class="lg:grid grid-cols-3 gap-4 items-center">
          <!-- Gender Name -->
          <label for="name"
            >Gender Name <span class="text-red-600">*</span></label
          >
...
const submitForm = async () => {
  if (validateForm()) {
    loading.value = true;
    try {
      const response = await gender.insertGender(form.value);

      if (response?.status === 201) {
        showNotification(
          "success",
          response?.data?.message || "Gender successfully created."
        );
      ...
      if (error.response) {
        showNotification(
          "error",
          error.response.data.message || "Failed to create gender."
        );
