  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Organization Type</h6>
...
import organizationType from "@/stores/setting/organization_type_api.js";
...
const fetchOrganizationTypeDetails = async () => {
  try {
    const { id } = route.params;
    const response = await organizationType.showOrganizationType(id);
    form.value = response.data;
  } catch (error) {
    console.error("Failed to fetch organization type details:", error);
  }
};
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await organizationType.updateOrganizationType(form.value, id);

    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      router.push({ name: "organization-type" });
    }
  }
  catch (error) {
    if (error.response) {
      showNotification("error", error.response.data.message || "Failed to update organization type");
...
onMounted(fetchOrganizationTypeDetails);
</script>
