  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit District</h6>
...
const validateForm = () => {
  const errors = {};
  if (!form.value.name) errors.name = "District Name is required";
...
const fetchDistrictDetails = async () => {
  try {
    ...
    const response = await district.showDistrict(id);
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await district.updateDistrict(form.value, id);

    // Check if the response status indicates success
    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      route.push({ name: "district" });
    }
  }
  catch (error) {
    ...
    if (error.response) {
      ...
      showNotification("error", error.response.data.message || "Failed to update district");
...
onMounted(fetchDistrictDetails);
</script>
