  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Influencing Role</h6>
...
const fetchInfluencingRoleDetails = async () => {
  try {
    const { id } = route.params;
    const response = await influencingRole.showInfluencingRole(id);
    form.value = response.data;
  } catch (error) {
    console.error("Failed to fetch influencing role details:", error);
  }
};
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await influencingRole.updateInfluencingRole(form.value, id);

    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      router.push({ name: "influencing-role" });
    }
  }
  catch (error) {
    if (error.response) {
      showNotification("error", error.response.data.message || "Failed to update influencing role");
...
onMounted(fetchInfluencingRoleDetails);
</script>
