...
const validateForm = () => {
  const errors = {};
  if (!form.value.name) errors.name = "BusinessIndustry Name is required";
...
const fetchBusinessIndustryDetails = async () => {
  try {
    ...
    const response = await businessIndustry.showBusinessIndustry(id);
    form.value = response.data;
  } catch (error) {
    console.error("Failed to fetch businessIndustry details:", error);
  }
};
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await businessIndustry.updateBusinessIndustry(form.value, id);

    // Check if the response status indicates success
    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      route.push({ name: "countries" });
    }
  }
  catch (error) {
    ...
    if (error.response) {
      ...
      showNotification("error", error.response.data.message || "Failed to update businessIndustry");
