  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Business Industry</h6>
...
import organizationType from "@/stores/setting/business-industry-api.js";
...
const fetchBusinessIndustryDetails = async () => {
  try {
    const { id } = route.params;
    const response = await organizationType.showBusinessIndustry(id);
    form.value = response.data;
  } catch (error) {
    console.error("Failed to fetch business industry details:", error);
  }
};
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await organizationType.updateBusinessIndustry(form.value, id);

    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      router.push({ name: "business-industry" });
    }
  }
  catch (error) {
    if (error.response) {
      showNotification("error", error.response.data.message || "Failed to update business industry");
...
onMounted(fetchBusinessIndustryDetails);
</script>
