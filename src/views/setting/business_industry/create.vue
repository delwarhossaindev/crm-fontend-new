...
const submitForm = async () => {
  if (validateForm()) {
    loading.value = true;
    try {
      const response = await businessIndustry.insertBusinessIndustry(form.value);
      ...
      if (response?.status === 201) {
        ...
        router.push({ name: "business-industries" });
      }
    } catch (error) {
      if (error.response) {
        showNotification(
          "error",
          error.response.data.message || "Failed to insert business industry"
        );
