  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center">
        <div class="mb-3">
          <h6 class="font-medium">Create New Business Industry</h6>
        </div>
        <div class="mb-3">
          <button
            type="button"
      ...
      <form @submit.prevent="submitForm">
        <div class="lg:grid grid-cols-3 gap-4 items-center">
          ...
          <label for="name"
            >Business Industry Name <span class="text-red-600">*</span></label
          >
...
function validateForm() {
  const errors = {};
  if (!form.value.name) errors.name = "Business Industry Name is required";
...
const submitForm = async () => {
  if (validateForm()) {
    loading.value = true;
    try {
      const response = await organizationType.insertBusinessIndustry(form.value);

      if (response?.status === 201) {
        showNotification(
          "success",
          response?.data?.message || "Business Industry successfully created."
        );
      ...
      if (error.response) {
        showNotification(
          "error",
          error.response.data.message || "Failed to create business industry."
        );
