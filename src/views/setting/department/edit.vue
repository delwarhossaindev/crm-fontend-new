  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Department</h6>
...
const validateForm = () => {
  const errors = {};
  if (!form.value.name) errors.name = "Department Name is required";
...
const fetchDepartmentDetails = async () => {
  try {
    ...
    const response = await department.showDepartment(id);
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await department.updateDepartment(form.value, id);

    // Check if the response status indicates success
    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      route.push({ name: "department" });
    }
  }
  catch (error) {
    ...
    if (error.response) {
      ...
      showNotification("error", error.response.data.message || "Failed to update department");
