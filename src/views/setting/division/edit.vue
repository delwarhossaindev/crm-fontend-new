  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Division</h6>
...
const fetchDivisionDetails = async () => {
  try {
    ...
    const response = await division.showDivision(id);
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await division.updateDivision(form.value, id);

    // Check if the response status indicates success
    if (response?.status === 200) {
      showNotification("success", response.data.message || "Updated successfully");
      route.push({ name: "division" });
    }
  }
  catch (error) {
    ...
    if (error.response) {
      ...
      showNotification("error", error.response.data.message || "Failed to update division");
...
onMounted(fetchDivisionDetails);
</script>
