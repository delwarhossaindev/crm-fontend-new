  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center">
        <div class="mb-3">
          <h6 class="font-medium">Create New Industry Type</h6>
        </div>
        <div class="mb-3">
          <button
            type="button"
      ...
      <form @submit.prevent="submitForm">
        <div class="lg:grid grid-cols-3 gap-4 items-center">
          <!-- IndustryType Name -->
          <label for="name"
            >Industry Type Name <span class="text-red-600">*</span></label
          >
...
function validateForm() {
  const errors = {};
  if (!form.value.name) errors.name = "Industry Type Name is required";
...
const submitForm = async () => {
  if (validateForm()) {
    loading.value = true;
    try {
      const response = await industryType.insertIndustryType(form.value);

      if (response?.status === 201) {
        showNotification(
          "success",
          response?.data?.message || "Industry Type successfully created."
        );
      ...
      if (error.response) {
        showNotification(
          "error",
          error.response.data.message || "Failed to create industry type."
        );
