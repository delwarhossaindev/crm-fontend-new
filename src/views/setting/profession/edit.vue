  ...
  <MainLayout>
    <div class="bg-white p-3 rounded-md">
      <div class="flex justify-between items-center mb-3">
        <h6 class="font-medium">Edit Profession</h6>
...
const fetchProfessionDetails = async () => {
  try {
    ...
    const response = await profession.showProfession(id);
    form.value = response.data;
  } catch (error) {
    console.error("Failed to fetch profession details:", error);
  }
};
...
const submitForm = async () => {
  ...
  try {
    const { id } = route.params;
    const response = await profession.updateProfession(form.value, id);
    ...
    if (error.response) {
      ...
      showNotification("error", error.response.data.message || "Failed to update profession");
...
onMounted(fetchProfessionDetails);
</script>
