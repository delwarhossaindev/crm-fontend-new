...
const deleteItem = async (index) => {
  ...
  try {
    const response = await item.deleteItem(itemId);
...
<template>
  <MainLayout>
    <div class="flex justify-between mb-4">
      <input type="text" placeholder="Search Item..." class="px-4 py-2 border rounded"
    ...
    <table class="table border-collapse border border-slate-400 w-full bg-white my-4">
      ...
      <tbody class="table-body">
        ...
        <tr v-if="!isLoading && !itemData?.length">
          <td colspan="5" class="text-red-600">No Item Found . . .</td>
        </tr>
        <tr v-for="(item, index) in itemData" :key="index" class="hover:bg-gray-100 transition-colors duration-200">
          ...
          <td class="font-bold">{{ allItems?.from + index }}</td>
    ...
    <a-pagination v-model:current="page" v-model:page-size="paginate" :total="allItems?.total"
      :show-total="(total) => `Total ${total} items`" @change="handlePagination(page)" />
  </MainLayout>
</template>
