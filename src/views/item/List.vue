<template>
  <MainLayout>
    ...
    <h1 class="title">All Items ({{ allData?.total || 0 }})</h1>
    <table class="table text-sm border-collapse border border-slate-400 w-full bg-white my-4">
      ...
      <tbody class="table-body">
        ...
        <tr v-for="(item, index) in allData?.data" :key="index">
          ...
          <td>{{ item.personal_info.model || "-" }}</td>
          <td>{{ item.position || "-" }}</td>
...
const fetchData = async () => {
  ...
  try {
    const response = await item.fetchItemList(page.value, itemsPerPage.value, searchQuery.value);
